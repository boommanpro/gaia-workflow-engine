 ...
 * 小于操作符实现
 */
public class LessThanOperator implements ConditionOperator<Object> {
    ...
    public boolean apply(Object left, Object right) {
        ...
        try {
            ...
            if (convertedLeft instanceof Comparable && convertedRight instanceof Comparable) {
                // 确保两个对象类型相同才进行比较
                if (convertedLeft.getClass().equals(convertedRight.getClass())) {
                    return ((Comparable) convertedLeft).compareTo(convertedRight) < 0;
                } else {
                    // 尝试转换为BigDecimal进行数字比较
